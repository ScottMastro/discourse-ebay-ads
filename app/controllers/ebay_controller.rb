...
class EbayAdPlugin::EbayController < ::ApplicationController
  ...
  def search
    ...
    base_query =
      EbayAdPlugin::EbayListing.joins(
        "INNER JOIN ebay_sellers ON ebay_listings.seller = ebay_sellers.ebay_username",
    ...
    if search_keys.present?
      base_query =
        base_query.where(
          "ebay_listings.title ILIKE :search OR ebay_listings.description ILIKE :search",
          search: "%#{search_keys}%",
        )
    end
    ...
    listing_hashes =
      paginated_listings.map do |listing|
  ...
  def info
    ...
    listing_statistics = { total_listings: total_listings, listings_by_seller: listings_by_seller }
  ...
  def update_user
    total_calls_today =
      EbayAdPlugin::EbayApiCall.where(date: Date.today, call_type: "browse").sum(:count)
    ...
    if seller.nil? || seller.ebay_username.blank?
      return(
        render json: {
                 status: "failed",
                 message: "No eBay account associated with username: #{username}",
               }
      )
    end
    ...
    if user_meets_criteria?(user, ebay_username)
      ...
      render json: {
               ...
               discourse_user: user.username,
               ebay_username: ebay_username,
             }
    else
      render json: {
               status: "failed",
               message: "User does not meet group/trust criteria or is blocked: #{username}",
             }
    end
  end
  ...
  def get_all_account_info
    account_info =
      EbayAdPlugin::EbaySeller.find_each.map do |seller|
        if seller.user_id.nil?
          next
        else
          user = User.find_by(id: seller.user_id)
          username = user.nil? ? nil : user.username
          { user_id: seller.user_id, username: username, ebay_username: seller.ebay_username }
        end
      end
  ...
  def get_api_calls
    EbayAdPlugin::EbayApiCall
      .select(:date, :count, :call_type)
      .where(date: Date.today)
      .map do |api_call|
        { date: api_call.date, count: api_call.count, call_type: api_call.call_type }
      end
  end
end
