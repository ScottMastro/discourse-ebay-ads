...
class EbayAdPlugin::EbaySellerController < ::ApplicationController
  ...
  def add_seller
    ...
    render json: {
             status: "failed",
             message: "Ebay seller was not added.",
           },
  ...
  def remove_seller
    ...
    if ebay_seller
      if ebay_seller.destroy
        ...
        render json: {
                 status: "failed",
                 message: "Ebay seller could not be removed.",
                 errors: ebay_seller.errors.full_messages,
               },
               status: :unprocessable_entity
      end
    else
  ...
  def get_user_settings
    ...
    if seller
      render json: { seller: seller.as_json(only: %i[user_id ebay_username hidden]) }
    else
  ...
  def all_seller_info
    sql = <<~SQL
          SELECT es.*, u.username AS user_username,
                 COALESCE(lc.cnt, 0) AS listings_count
          ...
          LEFT JOIN (
            ...
            WHERE active = TRUE
            GROUP BY seller
          ) lc ON lc.seller = es.ebay_username
  ...
  def unblock_seller
    ...
    if seller.nil?
      render json: {
               status: "failed",
               error: "Seller #{ebay_username} is unknown.",
             },
             status: :not_found
      return
    elsif seller.blocked
  ...
  def blocklist
    ...
    sellers_info =
      blocked_sellers.map do |seller|
        { username: seller.username, ebay_username: seller.ebay_username, blocked: seller.blocked }
      end
  ...
  def dump_seller_listings
    ...
    render json: {
             status: "ok",
             message: "Dropping eBay listings from #{ebay_username} (job triggered)",
           }
  end
end
