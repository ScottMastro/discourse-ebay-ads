...
export default class AdminPluginsEbayController extends Controller {
  ...
  _addSeller(ebay_username, blocked) {
    if (ebay_username !== "") {
      ...
      ajax(`/ebay/seller/add/${encodedSeller}.json`)
        .then((result) => {
          if (result.status === "ok") {
            const index = this.allSellers.findIndex(
              (seller) => seller.ebay_username === ebay_username
            );
            if (index === -1) {
              const newSeller = {
                username: null,
          ...
          if (result.status === "ok") {
            this.allSellers.forEach((seller) => {
              if (seller.ebay_username === ebay_username) {
                const index = this.allSellers.findIndex(
                  (s) => s.ebay_username === ebay_username
                );
                if (index !== -1) {
                  let modifiedSeller = Object.assign(
                    {},
                    this.allSellers[index]
                  );
  ...
  deleteSellerFromTable(ebay_username) {
    let encodedUsername = encodeURIComponent(ebay_username);
    encodedUsername = encodedUsername.replace(/\./g, "%2E");
    ajax(`/ebay/seller/remove/${encodedUsername}`)
      .then((result) => {
        if (result.status === "ok") {
          this.allSellers = this.allSellers.filter(
            (seller) => seller.ebay_username !== ebay_username
          );
        }
      })
  ...
  fetchListings(username) {
    ...
    ajax(`/ebay/user/update/${encodedUser}.json`).catch(popupAjaxError);
  }
}
